...
| `openkb` | Use OpenKB as a compiled markdown knowledge base alongside the `openviking` memory provider. Orients on schema/index/log, queries structured recall, files durable answers, and can operate against local or SSH-hosted OpenKB runtimes. Configurable via `skills.config.openkb.*`. | `research/openkb` |
